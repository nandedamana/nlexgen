...
void trie_node_add(TrieNode *this, const char * key, int keyoffset, const char * action, _Bool lengthwise)
{
	...
	if(lengthwise) {
		...
		while(chld) {
			...
			chld = (TrieNode *) trie_node_get_next(chld);
		}

		if((!chld) || (chld->keylen != klen)) {
			TrieNode *_tmp_1 = (TrieNode *) malloc(sizeof(TrieNode));
			if(_tmp_1 == NULL) {
				perror(NULL);
				exit(EXIT_FAILURE);
			}

			trie_node_construct(_tmp_1);
			chld = _tmp_1;
			chld->keylen = klen;
...
_Bool is_fastkeyword(const char * pattern)
{
	...
	for(_ngg_tmp_0 = 0u; _ngg_tmp_0 < (strlen(pattern)); _ngg_tmp_0 += 1u) {
